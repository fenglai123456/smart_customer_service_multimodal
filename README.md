...
> 说明：本项目以选修课大作业和课堂演示为目标。报告中的准确率来自课程 demo 测试集，用于证明系统链路可运行、五类场景可识别，并具备一定泛化能力；后续可继续扩展更多真实业务样本做补充验证。
...
2. 语音分支：音频文件提取 MFCC/RMS/过零率/谱质心等声学特征，优先调用 LSTM 模型，轻量运行环境下使用 MFCC 统计分类器作为补充。
...
为了让仓库适合 GitHub 查看，本仓库上传了课程 demo 运行所需的模型、训练脚本和指标文件，较大的训练中间文件与原始数据集可按脚本重新生成。
...
本项目已经完成课程演示所需的主要功能：多模态输入、五类意图识别、截图五分类、融合模型、前端展示、测试脚本和报告素材。系统在课程 demo 测试集上运行稳定，并具备一定泛化能力；后续可继续补充更多客服截图、语音和对话样本，进一步扩大验证范围。
