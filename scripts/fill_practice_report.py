...
OUT = Path(r"C:\Users\ASUS\Downloads\基于多模态的意图感知人机交互智能体 实践报告_刘武_GitHub版.docx")
...
def build_report(doc: Document):
    ...
    for text in [
        ...
        "通过课程演示数据进行端到端测试，分析多模态融合带来的效果，并总结项目开发过程中的问题与优化方法。",
    ]:
        add_para(doc, text)
    ...
    add_para(doc, "需要说明的是，本报告中的准确率为课程演示数据和项目 demo 测试集上的结果，用于说明系统功能链路能够正常工作，不作为严格公开数据集泛化准确率证明，也不等同于真实业务部署中的大规模泛化性能。")
    ...
    for text in [
        ...
        "系统不足主要体现在：当前真实客服公开截图数据有限，截图 CNN 和融合 MLP 以课程 demo 数据为主；文本分支虽然包含 BERT 接口，但实际运行中仍以 TF-IDF/关键词增强保证稳定性；音频和表情分支受样本规模限制，更多承担辅助作用。后续如果要面向真实业务部署，应扩大真实多模态样本规模并进行严格交叉验证。",
    ]:
        add_para(doc, text)

    add_heading(doc, "五、实验难点与解决方法", 2)
    for text in [
        "难点1：多模态输入来源和格式差异大，直接拼接容易造成特征维度不统一。",
        ...
        "难点2：公开真实客服截图数据不足，单纯依赖小样本 CNN 容易影响课堂演示稳定性。",
        "解决思路：课程项目以功能演示为重点，因此采用 demo CNN + OCR/关键词规则 + 当前融合模型的方案，既展示截图五分类流程，又避免声称真实业务准确率已经严格达标。",
        "解决方法：为五类场景整理 course_demo_test_data 测试文件夹，保证每个样例具有不同截图、音频和表情素材；在报告中明确 demo 数据性质，并用端到端 demo 测试准确率作为课程汇报指标。",
        "难点3：前端需要同时展示业务结果和技术细节，信息过多时页面容易拥挤。",
    ...
    for text in [
        "在本次 24 学时的课外实践中，我体会到多模态人机交互并不是简单地把多个模型堆在一起，而是要先明确业务场景和用户需求，再决定每个模态承担什么作用。以智能客服为例，用户真正关心的是问题能否被准确分流和快速处理，因此系统不仅要输出意图分类结果，还要给出服务推荐、处理步骤和必要的人工介入判断。",
        ...
        "在实践能力方面，项目开发过程中遇到了很多工程问题，例如数据格式不统一、模型输出标签不一致、前端上传预览不直观、融合过程难以解释等。通过不断调试，我逐步把系统拆成数据处理、单模态识别、融合决策、前端展示和测试验证几个模块，使问题定位更清晰，也提升了系统设计能力。",
        "同时我也认识到，目前系统更适合课程演示和原型验证，还不能直接等同于真实客服业务系统。真实业务中需要更多公开或企业内部授权的客服截图、真实语音和真实对话数据，还需要更严格的训练集/测试集划分、泛化测试和隐私合规处理。后续如果继续优化，我会优先扩充真实多模态样本，升级文本模型到更强的中文预训练模型，并对融合策略做更系统的对比实验。",
    ]:
        add_para(doc, text)
